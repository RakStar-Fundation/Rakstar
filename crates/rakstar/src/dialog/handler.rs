...
pub(crate) struct DialogWorker {
    sender: Option<oneshot::Sender<DialogResponse>>,
}

impl DialogWorker {
    pub(crate) fn new() -> (Self, oneshot::Receiver<DialogResponse>) {
        let (tx, rx) = oneshot::channel();
        (Self { sender: Some(tx) }, rx)
    }

    pub(crate) fn notify(&mut self, resp: DialogResponse) {
...
impl DialogManager {
    ...
    pub(crate) fn send(&mut self) -> (u32, oneshot::Receiver<DialogResponse>) {
        self.counter += 1;
...
pub(crate) static DIALOG_MANAGER: Lazy<Arc<Mutex<DialogManager>>> =
